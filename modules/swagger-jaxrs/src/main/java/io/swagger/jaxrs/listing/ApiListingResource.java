...
public class ApiListingResource {
		...
		if (scanner != null) {
			...
			if (classes != null) {
				...
				if (scanner instanceof ReaderConfig) {
					...
					readerConfig = ReaderConfigUtils.getReaderConfig(this.context);
				}
	...
	private Swagger process(
			...
			HttpHeaders headers,
			UriInfo uriInfo) {
		Swagger swagger = this.scan(app, sc);

		if (swagger != null) {
			SwaggerSpecFilter filterImpl = FilterFactory.getFilter();
			if (filterImpl != null) {
				SpecFilter f = new SpecFilter();
				swagger = f.filter(swagger, filterImpl, this.getQueryParams(uriInfo.getQueryParameters()), this.getCookies(headers),
